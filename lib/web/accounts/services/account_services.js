...
var AccountServices = {
    ...
    addStream: function(accountId, streamName, cb){
        ...
        exec(function(conn){
            async.waterfall([
                function doesAccountHaveStreams(callback){
                    r.table(accountsTable)
                     .get(accountId)
                     .hasFields('streams')
                     .run(conn, function(err, hasFields){
                        if(err)
                            callback(err, null);
                        else
                            callback(err, hasFields);
                     })
                },

                function addNewStreamToAccount(fieldStreamExists, callback){
                    if(fieldStreamExists) {
                        r.table(accountsTable)
                            .get(accountId)

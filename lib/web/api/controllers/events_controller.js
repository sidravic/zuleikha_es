...
var catchupStream = function(accountId, streamName, startSequenceId, endSequenceId, cb){
    HttpEventStreamCatchupService.fetch(accountId, streamName,
                                        startSequenceId, endSequenceId, cb)
};
                ...
                accountService.streamBelongsToaccount(streamName, accountEmail, function(err, belongsTo){
                    if(err){
                        ...
                        if(belongsTo){
                            ...
                            if(endSequenceId) {
                                debugger;
                                catchupStream(accountId, streamName, startSequenceId, endSequenceId, onResultSet);
                            }else{
                                accountService.getStreamEventCount(accountId, streamName, function(err, count){
                                    if(err){
                                        reply({
                                            requestStatus: 'failure',
                                            error: 'Internal Server Error',
